...
public class SampleController {
	...
	public String ex02(@RequestParam("owner") String name, @RequestParam("age") int age) {
		// 프론트 변수와 백엔드의 변수가 다를때 사용(초보자는 일치시키는게 좋다)
		log.info("name : " + name);
		log.info("age : " + age);
	...
	@GetMapping("/ex02Bean")
	public String ex02Bean(SampleDTOList list) { // int page는 게시판에 페이징기법 예시
		// url 경로에 배열로 들어오는 파라미터
		// /ex02Bean?list[0].name=kkw&lis[2].age=33
	...
	public void ex05() {
		...
		// 리턴이 void일 경우에는 자동으로 [/WEB-INF/views/sample/ex05.jsp]를 찾는다.
		// 초보자 사용 금지
	} 	// ex05() 메서드 종료
	...
	public ResponseEntity<String> ex07() {
		...
		header.add("Content-Type", "application/json;charset");
